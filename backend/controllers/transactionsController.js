...
const getTransactionById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ mensaje: "Ha ocurrido un error al obtener la transacción" });
  }
};
...
const createTransaction = async (req, res) => {
  ...
  try {
    ...
    if (senderUser.balance < amount) {
      return res
        .status(400)
        .json({ message: "El remitente no tiene suficiente saldo" });
    }
    ...
    res
      .status(500)
      .json({ message: "Ha ocurrido un error al procesar la transaccion" });
  }
};
